...
public class Codepoint {
	 ...
	 *
	 * @param hexadecimalString the string of the hexidecimal codepoint without the
	 *                          header
	 */
	...
	/**
	 * Converts the codepoint to UTF8. Depending on where the codepoint is in the
	 * range of values, the UTF8 value will be between 1 and 4 bytes
	 *
	...
	public String toUTF8() {
		...
		if (parsedCodePointAsInt >= START_RANGE_1_BYTE_UTF8 && parsedCodePointAsInt <= END_RANGE_1_BYTE_UTF8) {

			return this.to1ByteUTF8();
		}
		if (parsedCodePointAsInt >= START_RANGE_2_BYTE_UTF8 && parsedCodePointAsInt <= END_RANGE_2_BYTE_UTF8) {

			return this.to2ByteUTF8();
		}

		if (parsedCodePointAsInt >= START_RANGE_3_BYTE_UTF8 && parsedCodePointAsInt <= END_RANGE_3_BYTE_UTF8) {

			return this.to3ByteUTF8();
		} else {
			return this.to4ByteUTF8();
		}

	}

	/**
	 * Converts the codepoint to UTF16. Depending on where the codepoint is in the
	 * range of values, the UTF16 value will be 2 or 4 bytes
	 *
	 * @return String the UTF16 encoding of the codepoint
	 */
	public String toUTF16() {
		int parsedCodePointAsInt = Integer.parseUnsignedInt(this.codepoint, 16);
		if ((parsedCodePointAsInt >= START_RANGE_2_BYTE_UTF16_FIRST_GAP
				&& parsedCodePointAsInt <= END_RANGE_2_BYTE_UTF16_FIRST_GAP)
				|| (parsedCodePointAsInt >= START_RANGE_2_BYTE_UTF16_SECOND_GAP
						&& parsedCodePointAsInt <= END_RANGE_2_BYTE_UTF16_SECOND_GAP)) {
			return this.codepoint;
		}
		if (parsedCodePointAsInt >= START_RANGE_4_BYTE_UTF16 && parsedCodePointAsInt <= END_RANGE_4_BYTE_UTF16) {
			return this.to4ByteUTF16();
		}
		throw new IllegalArgumentException("Cannot convert this hexstring to UTF16");
	}
	...
	public String toUTF32() {
		...
		for (int count = 0; count < paddedZeros; count++) {
			output.insert(0, "0");
		}
	...
	private String getBinaryStringOfCodepoint() {
		...
		for (char character : this.codepoint.toCharArray()) {
			binaryString.append(String.format("%4s", Integer.toBinaryString((byte) Character.digit(character, 16)))
					.replace(" ", "0"));
		}
	...
	private String getBinaryStringOf(String thing) {
		StringBuilder binaryString = new StringBuilder();
		for (char character : thing.toCharArray()) {
			binaryString.append(String.format("%4s", Integer.toBinaryString((byte) Character.digit(character, 16)))
					.replace(" ", "0"));
		}
		return binaryString.toString();
	...
	private String to2ByteUTF8() {
		String byte1 = "110"
				+ this.getBinaryStringOfCodepoint().substring(5, this.getBinaryStringOfCodepoint().length() - 6);
		String byte2 = "10"
				+ this.getBinaryStringOfCodepoint().substring(this.getBinaryStringOfCodepoint().length() - 6);
		return this.convert8BytesTo2HexDigits(byte1) + this.convert8BytesTo2HexDigits(byte2);
	}

	private String to3ByteUTF8() {
		String byte1 = "1110"
				+ (this.getBinaryStringOfCodepoint().substring(0, this.getBinaryStringOfCodepoint().length() - 12));
		String byte2 = "10"
				+ this.getBinaryStringOfCodepoint().substring(4, this.getBinaryStringOfCodepoint().length() - 6);
		String byte3 = "10"
				+ this.getBinaryStringOfCodepoint().substring(this.getBinaryStringOfCodepoint().length() - 6);

		return this.convert8BytesTo2HexDigits(byte1) + this.convert8BytesTo2HexDigits(byte2)
				+ this.convert8BytesTo2HexDigits(byte3);
	}

	private String to4ByteUTF8() {
		String byte1 = "11110"
				+ this.getBinaryStringOfCodepoint().substring(3, this.getBinaryStringOfCodepoint().length() - 18);
		String byte2 = "10"
				+ this.getBinaryStringOfCodepoint().substring(6, this.getBinaryStringOfCodepoint().length() - 12);
		String byte3 = "10"
				+ this.getBinaryStringOfCodepoint().substring(12, this.getBinaryStringOfCodepoint().length() - 6);
		String byte4 = "10"
				+ this.getBinaryStringOfCodepoint().substring(this.getBinaryStringOfCodepoint().length() - 6);

		return this.convert8BytesTo2HexDigits(byte1) + this.convert8BytesTo2HexDigits(byte2)
				+ this.convert8BytesTo2HexDigits(byte3) + this.convert8BytesTo2HexDigits(byte4);
	}
	...
	private String to4ByteUTF16() {
		...
		String paddedBinaryOfHex = String.format("%" + 20 + "s", this.getBinaryStringOf(codepointBackToHex))
				.replace(' ', '0');
